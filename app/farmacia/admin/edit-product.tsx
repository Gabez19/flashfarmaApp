...
export default function EditProduct() {
  ...
  const [nome, setNome] = useState("Dipirona 500mg");
  const [descricao, setDescricao] = useState("Descrição do produto...");
  const [preco, setPreco] = useState("21.45");
  const [estoque, setEstoque] = useState("420");
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 40 }}>
        <View style={styles.card}>
          ...
          <TextInput placeholder="Descrição" style={[styles.input,{height:100,textAlignVertical:"top"}]} multiline value={descricao} onChangeText={setDescricao} />
          <View style={styles.row}>
            <TextInput placeholder="Preço" style={[styles.input, styles.half]} value={preco} onChangeText={setPreco} />
            <TextInput placeholder="Estoque" style={[styles.input, styles.half]} value={estoque} onChangeText={setEstoque} />
          </View>
          <TextInput placeholder="Categoria" style={styles.input} value={categoria} onChangeText={setCategoria} />
          <Text style={styles.label}>Imagem do Produto</Text>
          <TextInput placeholder="URL ou Base64" style={[styles.input,{height:150,textAlignVertical:"top"}]} multiline value={imagem} onChangeText={setImagem} />
          <View style={styles.buttonRow}>
            <TouchableOpacity onPress={() => router.back()}><Text style={styles.cancelText}>Cancelar</Text></TouchableOpacity>
            <TouchableOpacity style={styles.deleteButton} onPress={() => router.push("/farmacia/admin/exclue-product")}> <Text style={styles.deleteText}>Excluir</Text></TouchableOpacity>
            <TouchableOpacity style={styles.saveButton}><Text style={styles.saveText}>Salvar</Text></TouchableOpacity>
          </View>
        </View>
      </ScrollView>
