// app/farmacia/admin/exclue-product.tsx
...
export default function ExclueProduct() {
  const router = useRouter();
  const params = useLocalSearchParams();
  const productId = params.id as string; // Obtém o ID da rota (Ex: /exclue-product?id=1)
  const [menuOpen, setMenuOpen] = useState(false);
  
  // O nome pode ser passado como parâmetro ou ser buscado com GET,
  // mas para simplificar aqui vamos apenas usar o ID para o DELETE.
  ...
  const handleDeleteProduct = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Redireciona para a lista de catálogo após a exclusão
        router.replace("/farmacia/admin/manage-catalog"); 
      } else {
        // Exibe o erro da API (e.g., "Produto não encontrado")
